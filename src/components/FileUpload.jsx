...
const FileUpload = () => {
  ...
  const handleFileChange = async (event) => {
    ...
    if (file) {
      setFileInfo(file);
      const response = await getTextFromPDF(file);
      if (response.success) {
  ...
  return (
    <Box sx={{ width: '100%', maxWidth: 800, mx: 'auto', textAlign: 'center', mt: 8 }}>
      <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', mb: 4 }}>
        ...
        {pdfText && (
          <Box sx={{ textAlign: 'left', mb: 2 }}>
            <Typography variant="body1"><strong>Extracted Text:</strong></Typography>
            <Typography variant="body2" sx={{ whiteSpace: 'pre-wrap' }}>{pdfText}</Typography>
          </Box>
        )}
